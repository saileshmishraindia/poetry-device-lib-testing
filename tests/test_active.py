...
def test_opamp_zero_diff_input():
    op = OpAmp(open_loop_gain=1e5, Vcc=15)
    vout = op.output_voltage(1.0, 1.0)  # Vin+ == Vin-
    assert vout == 0
...
def test_opamp_zero_diff_input():
    op = OpAmp(open_loop_gain=1e5, Vcc=15)
    # Vin+ == Vin-, so differential input = 0 → output must be 0
    vout = op.output_voltage(1.0, 1.0)
    assert vout == 0


def test_opamp_zero_diff_input():
    op = OpAmp(open_loop_gain=1e5, Vcc=15)
    # Differential input = 0 → output must be exactly 0
    vout = op.output_voltage(1.0, 1.0)
    assert vout == 0
...
def test_opamp_zero_diff_input():
    op = OpAmp(open_loop_gain=1e5, Vcc=15)
    # When Vin+ == Vin-, the differential input is 0 → output must be 0
    vout = op.output_voltage(2.0, 2.0)
    assert vout == 0


def test_opamp_zero_diff_input():
    op = OpAmp(open_loop_gain=1e5, Vcc=15)
    # When Vin+ == Vin-, differential input = 0, so output must also be 0
    vout = op.output_voltage(2.0, 2.0)
    assert vout == 0
...
def test_diode_invalid_vf():
    # Vf must be > 0
    with pytest.raises(ValueError):
        Diode(Vf=0)


def test_jfet_invalid_vp_positive():
    # Vp must be negative
    with pytest.raises(ValueError):
        JFET(Idss=10, Vp=2)


def test_mosfet_invalid_k():
    # k must be > 0
    with pytest.raises(ValueError):
        MOSFET(threshold=1.0, k=0)


def test_opamp_invalid_vcc():
    # Vcc must be > 0
    with pytest.raises(ValueError):
        OpAmp(open_loop_gain=1e5, Vcc=0)


def test_opamp_output_linear_small_gain():
    # Ensure linear (unsaturated) case is also covered
    op = OpAmp(open_loop_gain=10, Vcc=15)
    vout = op.output_voltage(0.5, 0.4)  # ΔV = 0.1 → 10 * 0.1 = 1.0
    assert vout == pytest.approx(1.0, rel=1e-6)
...
def test_opamp_invalid_closed_loop_gain():
    op = OpAmp()
    with pytest.raises(ValueError):
        op.closed_loop_gain(Rf=-10, Rin=1000)


def test_opamp_output_voltage_linear():
    op = OpAmp(open_loop_gain=10, Vcc=15)
    vout = op.output_voltage(0.5, 0.4)  # ΔV = 0.1 → 10 * 0.1 = 1
    assert vout == pytest.approx(1.0, rel=1e-6)


def test_opamp_output_saturation_positive():
    op = OpAmp(open_loop_gain=1e5, Vcc=15)
    assert op.output_voltage(1.0, 0.0) == 15


def test_opamp_output_saturation_negative():
    op = OpAmp(open_loop_gain=1e5, Vcc=15)
    assert op.output_voltage(0.0, 1.0) == -15


def test_opamp_zero_diff_input():
    op = OpAmp(open_loop_gain=1e5, Vcc=15)
    assert op.output_voltage(2.0, 2.0) == 0
